...
-e s/\${s3jsonpathsfile}/$s3jsonpathsfile/ \
-e s/\${iamrole}/$iamrole/ \
-e s/\${stage}/$stage/"

startSQL="$(cat "./$object_$stage/start.sql" | sed -n $sedSubs)"
finishedSQL="$(cat "./$object_$stage/finished.sql" | sed -n $sedSubs)"
errorSQL="$(cat "./$object_$stage/error.sql" | sed -n $sedSubs)"
